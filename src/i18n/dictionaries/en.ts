...
export const en: Dictionary = {
  ...
  feeds: {
    ...
    scheduler: {
      ...
      lastRepaired: (n: number) => `${n} repaired`,
    },
    kicksdb: {
  ...
  repair: {
    title: "Repair products on the store",
    tag: "non-destructive",
    hint: "Puts back what products already online are missing — photo, brand, category, gender — from the source that owns them. It fills EMPTY fields only: a photo you uploaded by hand or a category you curated is never touched. No variation is deleted, and running it again changes nothing.",
    ...
    liveTitle: "Repair done",
    repaired: (n: number) => `${n} repaired`,
    whole: (n: number) => `${n} already whole`,
    failedCount: (n: number) => `${n} failed`,
    andMore: (n: number) => `…and ${n} more`,
    failed: "Repair failed",
    unavailableHint: "Missing on the store, but the source does not have it either: nothing to write.",
    fields: { image: "photo", brand: "brand", category: "category", gender: "gender" },
  },
  taxonomy: {
    title: "Taxonomies",
    desc: "Where products land on the store, and which identity fields get written. Every edit is re-run against the catalog you actually have: you see the effect before saving, not after publishing.",
    sourcesTitle: "Who decides the category",
    sourcesHint:
      "Some sources ship a curated taxonomy (KicksDB sends category and secondary category as real fields): that one can be written as-is. A supplier feed ships none — the tree you see in the catalog is inferred from the title, and writing it would duplicate the brands and mint a term per model. For those sources the rules below decide.",
    noSources: "No catalog products yet: sync a feed to see its sources.",
    modeTree: "Source tree",
    modeRules: "Rules",
    products: (n: number) => `${n} products`,
    rulesTitle: "Category rules",
    rulesHint:
      "Same behaviour as the margin rules: the most SPECIFIC rule wins (a SKU beats a brand, which beats the source). With no rule matching, the default category applies.",
    defaultCategory: "Default category",
    defaultPlaceholder: "Sneakers",
    defaultHint:
      "Use \" > \" to nest: \"Apparel > T-shirts\". Leave it empty to write no category at all — the store will use its own default, which is more honest than a label this app invented.",
    ...
    scopeSource: "Source",
    scopeBrand: "Brand",
    scopeFamily: "Catalog family",
    scopeName: "Name contains",
    scopeNamePlaceholder: "hoodie, tee, jacket…",
    scopeSku: "Exact SKU",
    ruleCategory: "Store category",
    ruleCategoryPlaceholder: "Apparel",
    fieldsTitle: "Fields written to the store",
    fieldsHint:
      "Turn off what you manage elsewhere: anything unticked is never written, on publish or on repair.",
    writeBrandTaxonomy: "Product brands (taxonomy)",
    writeBrandTaxonomyHint: "WooCommerce's native taxonomy — the \"Product brands\" column.",
    writeBrandAttribute: "pa_brand attribute",
    writeBrandAttributeHint:
      "Some Merchant Center / TikTok plugins read the brand only from here, not from the taxonomy.",
    writeGender: "pa_gender attribute",
    writeGenderHint:
    ...
    save: "Save",
    saving: "Saving…",
    reset: "Restore the shipped defaults",
    upToDate: "Saved",
    failed: "Something went wrong",
    appliesHint:
      "Applies to future publishes. For products already online use \"Repair products on the store\" in the Publish tab.",
  },
  publish: {

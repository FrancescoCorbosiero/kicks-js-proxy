...
export const en: Dictionary = {
  ...
  sync: {
    ...
    create: {
      ...
      tag: "new products from the feeds",
      desc: "Create on the store the products the sources know but your Woo doesn't have yet (mostly GoldenSneakers). Each is born from its owner's data with the same clean identity: EU sizes, standard SKUs, GTINs, prices, real stock (GS). The parent is minimal — name, SKU, pa_taglia — WITHOUT images by default (WP sideloads them slowly) and as a draft, so a run never dumps bare products onto the storefront.",
      compute: "Compute missing products",
      scope: (gs, kx) => `${gs} from GoldenSneakers${kx > 0 ? ` · ${kx} from KicksDB (excluded)` : ""}`,
      includeKicksdb: (n) => `Also include ${n} from KicksDB`,
      statusLabel: "Status",
      draft: "Draft (recommended)",
      publish: "Published (live immediately)",
      images: "Include images",
      imagesHint: "WordPress sideloads each image at creation time — slow across many products. Prefer the dedicated media phase.",
      create: (n) => `Create ${n} products`,
      dryTitle: (n, publish) => `Would create ${n} products${publish ? " (PUBLISHED)" : " (draft)"}:`,
      sizesCount: (n) => `${n} sizes`,
      progress: (done, total) => `${done}/${total} products`,
      doneLine: (created, variations, failed) =>
        `${created} created · ${variations} variations${failed > 0 ? ` · ${failed} failed` : ""}`,
